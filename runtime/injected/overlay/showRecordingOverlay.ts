...
import { RECORDING_OVERLAY_STYLES } from '#recorder-runtime/injected/overlay/styles.ts'
import type { AriaRuntime } from '@te/aria/browser'

export { showRecordingOverlay }

function showRecordingOverlay(args: ShowRecordingOverlayArgs, ariaRuntime: AriaRuntime): void {
  if (document.querySelector(`[${args.recorderUiAttribute}]`)) {
    return
  }
  if (!document.documentElement) {
    document.addEventListener('DOMContentLoaded', () => showRecordingOverlay(args, ariaRuntime), { once: true })
    return
  }
  ...
  function render(): void {
    ...
    if (locatorElement !== hoveredElement) {
      ...
      const locator = generateLocatorCandidates(hoveredElement, generateSelectorCandidates, ariaRuntime)[0]
