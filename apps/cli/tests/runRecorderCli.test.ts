import { runRecorderCli } from '#cli/runRecorderCli/index.ts'
import { mkdir, readFile, readdir } from 'node:fs/promises'
import { join } from 'node:path'
import { describe, expect, test } from 'vitest'

import { createRecording, serializeRecording, type CapturedRecording, type RecordedAriaSnapshot, type Recording } from '@te/recorder-recording'

import { useTemporaryDirectories } from './support/temporaryDirectories.ts'

describe('runRecorderCli', () => {
  const temporaryDirectories = useTemporaryDirectories()

  test('passes the requested start URL to the recorder', async () => {
    expect((await runCliRecording(await temporaryDirectories.create())).recordedStartUrl).toBe('https://example.com')
  })

  test('uses a collision-safe recording directory', async () => {
    const result = await runCliRecording(await temporaryDirectories.create())

    expect(await readdir(result.workingDirectory)).toContain('example-2.recording')
  })

  test('saves raw events and their projected steps', async () => {
    const result = await runCliRecording(await temporaryDirectories.create())

    expect(JSON.parse(await readFile(join(result.directoryPath, 'recording.json'), 'utf8'))).toEqual(JSON.parse(serializeRecording(result.recording)))
    expect(JSON.parse(await readFile(join(result.directoryPath, 'steps.json'), 'utf8'))).toMatchObject([
      { kind: 'goto', url: 'https://example.com' },
      { kind: 'fill', value: { kind: 'plain-text', value: 'hi' } },
    ])
  })

  test('uses the final input event preview for a projected fill', async () => {
    const result = await runCliRecording(await temporaryDirectories.create())

    expect(JSON.parse(await readFile(join(result.directoryPath, 'snapshots', '0001.aria.json'), 'utf8'))).toEqual(result.finalPreview.snapshot)
    expect(Array.from(await readFile(join(result.directoryPath, 'snapshots', '0001.png')))).toEqual(Array.from(result.finalPreview.screenshot))
  })

  test('opens the saved recording in the editor', async () => {
    const result = await runCliRecording(await temporaryDirectories.create())

    expect(result.openedDirectories).toEqual([result.directoryPath])
  })
})

async function runCliRecording(workingDirectory: string): Promise<CliRecordingResult> {
  await mkdir(join(workingDirectory, 'example.recording'))
  const recording: Recording = {
    ...createRecording({ startUrl: 'https://example.com', title: 'Example recording' }),
    events: [
      { kind: 'goto', pageUrl: 'about:blank', url: 'https://example.com' },
      { inputValue: 'h', key: 'h', kind: 'key-press', locatorCandidates: [{ framePath: ['#frame'], kind: 'css', value: '#search' }], pageUrl: 'https://example.com' },
      { inputValue: 'hi', key: 'i', kind: 'key-press', locatorCandidates: [{ value: '#search', kind: 'css', framePath: ['#frame'] }], pageUrl: 'https://example.com' },
    ],
  }
  const firstPreview = { screenshot: Uint8Array.from([1]), snapshot: inputSnapshot('h') }
  const finalPreview = { screenshot: Uint8Array.from([2]), snapshot: inputSnapshot('hi') }
  const capture: CapturedRecording = {
    events: [{ event: recording.events[0] }, { event: recording.events[1], preview: firstPreview }, { event: recording.events[2], preview: finalPreview }],
    metadata: { createdAt: recording.createdAt, startUrl: recording.startUrl, title: recording.title },
  }
  const openedDirectories: string[] = []
  let recordedStartUrl: string | undefined

  await runRecorderCli({
    argv: ['record', 'https://example.com'],
    recorder: {
      dispose: async () => undefined,
      start: async args => {
        recordedStartUrl = args.startUrl
        await args.onStopRequested?.()
      },
      stop: async () => capture,
    },
    runRecordingEditor: async args => {
      openedDirectories.push(args.directoryPath)
    },
    stdout: { write: () => undefined },
    waitForStop: signal =>
      new Promise(resolve => {
        signal.addEventListener('abort', () => resolve(), { once: true })
      }),
    workingDirectory,
  })

  return { directoryPath: join(workingDirectory, 'example-2.recording'), finalPreview, openedDirectories, recordedStartUrl, recording, workingDirectory }
}

function inputSnapshot(value: string): RecordedAriaSnapshot {
  return { children: [{ name: value, props: {}, ref: 'e1', role: 'textbox', target: true }], name: '', props: {}, role: 'fragment' }
}

interface CliRecordingResult {
  directoryPath: string
  finalPreview: { screenshot: Uint8Array; snapshot: RecordedAriaSnapshot }
  openedDirectories: string[]
  recordedStartUrl: string | undefined
  recording: Recording
  workingDirectory: string
}
