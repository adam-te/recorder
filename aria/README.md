...
Browser-side utilities for generating Playwright-compatible AI ARIA snapshots and semantic locator candidates. Snapshots are JSON-safe tree DTOs containing per-node `ariaVisible` state; `renderAriaSnapshot` reconstructs Playwright's AI-mode YAML from a stored tree.
